...
//- (UIView *)view
//{
//  return [[PayFortView alloc] initWithFrame:CGRectMake(UIScreen.mainScreen.bounds.origin.x,
...
//    self.payFort  = [[PayFortController alloc]initWithEnviroment:PayFortEnviromentProduction];
    self.payFort  = [[PayFortController alloc]initWithEnviroment:PayFortEnviromentSandBox];


    NSMutableString *post = [NSMutableString string];
    [post appendFormat:@"%@access_code=%@",shaPhrase, access_code];
    [post appendFormat:@"device_id=%@",   self.payFort.getUDID];
    ...
    NSDictionary* tmp = @{ @"service_command": @"SDK_TOKEN",
                           ...
                           @"language": language,
                           @"device_id": self.payFort.getUDID};
    NSError *error;
    NSData *postdata = [NSJSONSerialization dataWithJSONObject:tmp options:0 error:&error];
    NSString *BaseDomain = @"https://sbpaymentservices.payfort.com/FortAPI/paymentApi"; //this url will change in production
...
//
//    PayFortController *PayFort = [[PayFortController alloc]initWithEnviroment:[isLive boolValue] ? PayFortEnviromentProduction : PayFortEnviromentProduction ];
  dispatch_async(dispatch_get_main_queue(), ^{
    if(self.payFort==nil){
    self.payFort = [[PayFortController alloc]initWithEnviroment:PayFortEnviromentSandBox];
    }
   ...
   // [self.payFort setPayFortCustomViewNib:@"PayFortView2"];

//    [PayFort setPayFortCustomViewNib:@"PayFortView"];
    self.payFort.isShowResponsePage = YES;
//    PayFort.hideLoading = YES;
    self.payFort.presentAsDefault = YES;
   // [PayFort setPayFortCustomViewNib:@""];

        UIViewController *nav =  (UIViewController*)[UIApplication sharedApplication].delegate.window.rootViewController;
    UIViewController *aController = [self topViewController];
//    [aController presentViewController:[UIViewController new] animated:false completion:nil];
        NSLog(@"Nav=%@",self);
        [ self.payFort callPayFortWithRequest:request currentViewController:aController
...
//    completion(PKPaymentAuthorizationStatusSuccess);
    ...
    if(asyncSuccessful) {
        ...
        PayFortController *PayFort = [[PayFortController alloc]initWithEnviroment:[isLive boolValue] ? PayFortEnviromentSandBox : PayFortEnviromentSandBox];
